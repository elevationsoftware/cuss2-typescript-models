...
/**
 * Interface Return Codes definitions with the following semantics...<p>  *ACK_OK:* Directive has been accepted by the CUSS platform and will be processed. - Check listener for asynchronous result(s). <br>  *ACK_ERROR:* Any error occurred when calling the endpoint. - Check listener for detailed error/status code. <br> *ACK_OAUTH_ERROR:* Application passed an invalid token when trying to execute the directive. <br> *ACK_PARAMETER:* Error in parameters, such as a missing component ID, missing or incorrect data. 
 * @export
 * @enum {string}
 */
